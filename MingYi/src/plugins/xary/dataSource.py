...
def get_subscription_link(user_id: str):
    ...
    try:
        subscription_links = requests.get("https://nanocore.top/api/v1/adminuser/user/fetch?pageSize=150&current=1",
...
def apply_subscription_link(user_id: str, nickname: str):
    ...
    try:
        apply_sublink = requests.post("https://nanocore.top/api/v1/adminuser/user/generate",
                                      headers=headers,
                                      data={"email_suffix": "nanocore.top",
        ...
        if apply_sublink["data"] == "true" or apply_sublink["data"] is True:
            subscription_info_list = requests.get(
                "https://nanocore.top/api/v1/adminuser/user/fetch?pageSize=150&current=1",
                headers=headers).json()
            subscription_info = {}
            for subscription_info_json in subscription_info_list["data"]:
                if subscription_info_json["email"] == f"{nickname}@nanocore.top":
                    subscription_info = subscription_info_json
                    break
            if subscription_info == {}:
                return {"status": "error", "message": "申请订阅链接失败。"}
            subscription_info["remarks"] = "{" + f'"qq": "{user_id}"' + "}"
            update_result = requests.post("https://nanocore.top/api/v1/adminuser/user/update", headers=headers,
