...
- 群消息记录：管理员可按群开启消息归档，并查询基础群统计和文本热词；管理员也可私聊指定群号测试查询。
...
| `.配置 配置项 值` | 管理员热修改当前群配置；支持每日新闻、火烧云城市、AI 联网、默认角色、复读阈值和群消息记录。 | `.配置 火烧云城市 上海` |
...
| `.群统计 时间范围` | 查看当前群基础统计；范围支持 `今日`、`7天`、`30天` 等。 | `.群统计 今日` |
| `.群热词 时间范围` | 查看当前群文本热词；范围支持 `今日`、`7天`、`30天` 等。 | `.群热词 7天` |
| `.群统计 群号 时间范围` | 管理员私聊机器人时，查看指定群基础统计，避免在群里打扰群友。 | `.群统计 123456 今日` |
| `.群热词 群号 时间范围` | 管理员私聊机器人时，查看指定群文本热词。 | `.群热词 123456 7天` |
...
群消息记录默认关闭，建议由管理员在需要的群里发送 `.群记录 开启` 后再开始入库。记录内容用于 `.群统计`、`.群热词` 以及后续词云、群画像等能力；插件会保存消息文本、消息段类型、发送者 QQ、群号、时间和有限的消息段 JSON。管理员私聊机器人时可以用 `.群统计 群号 今日`、`.群热词 群号 7天` 测试指定群的统计结果。

| 配置项 | 默认值 | 说明 |
| --- | --- | --- |
| `MESSAGE_ARCHIVE_ENABLED` | `false` | 群消息记录的默认开关。为保护隐私，建议保持关闭并按群开启。 |
| `MESSAGE_ARCHIVE_RETENTION_DAYS` | `90` | 默认保留天数；也可在群里用 `.群记录 保留 90` 覆盖。 |
| `MESSAGE_ARCHIVE_HOTWORD_LIMIT` | `12` | `.群热词` 返回的热词数量。 |
| `MESSAGE_ARCHIVE_QUERY_LIMIT` | `5000` | 单次热词统计最多读取的近期文本消息数。 |
...
| `.配置 每日新闻 开/关` | 覆盖全局每日新闻默认开关。 | `.配置 每日新闻 关` |
